...
# ============================================================
# Real-world ground station locations (lat_deg, lon_deg, name)
# Sources: NASA Near Earth Network, ESA Estrack
# These stations rotate with Earth each frame.
# TDRS relay satellites sit at GEO altitude (~35,786 km) and
# stay fixed in the scene since they are geostationary.
# ============================================================
...
TDRS_MARKER_R              = 0.040
TDRS_LABEL_H               = 10
GEO_ALTITUDE_M             = 35_786_000.0                 # metres above surface
GEO_RADIUS_SCENE           = (R_EARTH + GEO_ALTITUDE_M) / R_EARTH  # scene units

# Real ground station network coordinates
...
# TDRS constellation: 3 geostationary relay satellites
# Longitude positions match NASA operational TDRS-East, TDRS-West, TDRS-Guam
TDRS_DEF = [
    {"name": "TDRS-East\n41°W GEO",   "lon": -41.0},
    {"name": "TDRS-West\n171°W GEO",  "lon": -171.0},
    {"name": "TDRS-Guam\n174°E GEO",  "lon":  174.0},
...
def lat_lon_to_eci_surface(lat_deg, lon_deg, radius_scene=GROUND_STATION_SURFACE_OFFSET):
    """
    Convert geodetic lat/lon to ECI scene position on Earth's surface.
    Uses simplified spherical Earth (no oblateness).
    lat_deg: -90 to +90  (positive = North)
    lon_deg: -180 to +180 (positive = East)
    radius_scene: scene units from origin (1.0 = Earth surface, >1.0 = above)
    Returns vpython vector in scene coordinates.
    Note: Earth rotates in the main loop, so this position is fixed in
    the Earth body frame. We update it each frame to follow Earth's rotation.
    """
    lat = radians(lat_deg)
    lon = radians(lon_deg)
    x = radius_scene * cos(lat) * cos(lon)
    y = radius_scene * cos(lat) * sin(lon)
    z = radius_scene * sin(lat)
    return vector(x, y, z)


def lon_to_geo_eci_scene(lon_deg):
    """
    Convert GEO longitude to scene position at GEO altitude.
    GEO satellites sit on the equatorial plane (lat = 0).
    They are fixed relative to Earth's surface so we also
    rotate them with Earth each frame.
    """
    lon = radians(lon_deg)
    x = GEO_RADIUS_SCENE * cos(lon)
    y = GEO_RADIUS_SCENE * sin(lon)
    z = 0.0
    return vector(x, y, z)
...
for gs_def in GROUND_STATIONS_DEF:
    pos = lat_lon_to_eci_surface(gs_def["lat"], gs_def["lon"])
    marker = sphere(
        pos=pos,
    ...
    # Store lat/lon so we can recompute ECI position each frame as Earth rotates
...
# ── Build TDRS relay satellite visual objects ─────────────────────────────────
tdrs_objects = []
for tdrs_def in TDRS_DEF:
    ...
    # Draw a thin dashed ring at GEO to show the geostationary belt
    tdrs_objects.append({
        "name":    tdrs_def["name"],
...
# Draw a faint GEO belt ring so the altitude is visible
geo_belt_ring = ring(
    pos=vector(0, 0, 0),
    axis=vector(0, 0, 1),
...
# ── Ground station network label in HUD ──────────────────────────────────────
gsn_label = label(
...
print(f"TDRS relay satellites: {len(TDRS_DEF)} placed at GEO altitude ({GEO_ALTITUDE_M/1000:.0f} km).")
for gs in GROUND_STATIONS_DEF:
    print(f"  GS  | {gs['name'].replace(chr(10),' '):<28} lat={gs['lat']:+.1f}  lon={gs['lon']:+.1f}")
for td in TDRS_DEF:
    print(f"  TDRS| {td['name'].replace(chr(10),' '):<28} lon={td['lon']:+.1f}  GEO")


def update_ground_station_visuals(physical_time_s):
    """
    Recompute ground station and TDRS marker positions each frame
    to follow Earth's rotation in ECI space.
    Earth rotates at EARTH_ROTATION_RATE rad/s.
    We apply the cumulative rotation angle to each station's base lon.
    TDRS satellites are geostationary so they rotate identically.
    """
    earth_rotation_angle_rad = EARTH_ROTATION_RATE * physical_time_s
    for gs in ground_station_objects:
        # Rotate the longitude by Earth's current rotation angle
        rotated_lon = gs["lon_deg"] + math.degrees(earth_rotation_angle_rad)
        pos = lat_lon_to_eci_surface(gs["lat_deg"], rotated_lon)
        gs["marker"].pos = pos
        gs["label"].pos  = pos + norm(pos) * 0.12

    for tdrs in tdrs_objects:
        rotated_lon = tdrs["lon_deg"] + math.degrees(earth_rotation_angle_rad)
        pos = lon_to_geo_eci_scene(rotated_lon)
        tdrs["marker"].pos = pos
        tdrs["label"].pos  = pos + vector(0, 0, 0.15)


# ── Export ground station + TDRS config for dispatcher ───────────────────────
# Write a static JSON file that the C++ dispatcher reads at startup
# so it knows exact positions and names without recomputing from Python.
GROUND_NETWORK_CONFIG_PATH = "ground_network_config.json"
_ground_network_config = {
    "schema": "ground_network.config.v1",
    "note": "Static ground station and TDRS relay config for dispatcher.cpp LOS geometry.",
    "speed_of_light_mps": 299_792_458.0,
...
try:
    with open(GROUND_NETWORK_CONFIG_PATH, "w") as _f:
        json.dump(_ground_network_config, _f, indent=2)
    print(f"Ground network config written to: {GROUND_NETWORK_CONFIG_PATH}")
except Exception as _e:
    print(f"Warning: could not write ground_network_config.json: {_e}")
